...
void set_cplus_marker_for_demangling(int ch) { cplus_markers[0] = ch; }
...
static char *internal_cplus_demangle PARAMS((struct work_stuff *,
                                             const char *));

static int demangle_template_template_parm PARAMS((struct work_stuff * work,
                                                   const char **, string *));

static int demangle_template PARAMS((struct work_stuff * work, const char **,
                                     string *, string *, int, int));

static int arm_pt PARAMS((struct work_stuff *, const char *, int, const char **,
                          const char **));

static int demangle_class_name PARAMS((struct work_stuff *, const char **,
                                       string *));

static int demangle_qualified PARAMS((struct work_stuff *, const char **,
                                      string *, int, int));

static int demangle_class PARAMS((struct work_stuff *, const char **,
                                  string *));

static int demangle_fund_type PARAMS((struct work_stuff *, const char **,
                                      string *));

static int demangle_signature PARAMS((struct work_stuff *, const char **,
                                      string *));

static int demangle_prefix PARAMS((struct work_stuff *, const char **,
                                   string *));
...
static int demangle_nested_args PARAMS((struct work_stuff *, const char **,
                                        string *));
...
static void demangle_function_name PARAMS((struct work_stuff *, const char **,
                                           string *, const char *));

static void remember_type PARAMS((struct work_stuff *, const char *, int));

static void remember_Btype PARAMS((struct work_stuff *, const char *, int,
                                   int));
...
static int demangle_template_value_parm PARAMS((struct work_stuff *,
                                                const char **, string *,
                                                type_kind_t));

static int do_hpacc_template_const_value PARAMS((struct work_stuff *,
                                                 const char **, string *));

static int do_hpacc_template_literal PARAMS((struct work_stuff *, const char **,
                                             string *));
...
int cplus_demangle_opname(const char *opname, char *result, int options) {
  ...
  if (opname[0] == '_' && opname[1] == '_' && opname[2] == 'o' &&
      opname[3] == 'p') {
    /* ANSI.  */
  ...
  } else if (opname[0] == '_' && opname[1] == '_' && opname[2] >= 'a' &&
             opname[2] <= 'z' && opname[3] >= 'a' && opname[3] <= 'z') {
    if (opname[4] == '\0') {
      /* Operator.  */
      size_t i;
  ...
  } else if (len >= 3 && opname[0] == 'o' && opname[1] == 'p' &&
...
static int demangle_signature(struct work_stuff *work, const char **mangled,
                              string *declp) {
  int success = 1;
  int func_done = 0;
  int expect_func = 0;
...
static int demangle_template(struct work_stuff *work, const char **mangled,
                             string *tname, string *trawname, int is_type,
                             int remember) {
  int i;
  int r;
  int need_comma = 0;
  ...
  if (is_type) {
    ...
    if (**mangled == 'z') {
      ...
      is_java_array =
          (work->options & DMGL_JAVA) && strncmp(*mangled, "JArray1Z", 8) == 0;
      if (!is_java_array) {
  ...
  for (i = 0; i < r; i++) {
    ...
    /* z for template parameters */
      ...
      if (success && (r2 = consume_count(mangled)) > 0 &&
      ...
      success =
          demangle_template_value_parm(work, mangled, s, (type_kind_t)success);
...
static int arm_pt(struct work_stuff *work, const char *mangled, int n,
                  const char **anchor, const char **args) {
...
static int demangle_class_name(struct work_stuff *work, const char **mangled,
                               string *declp) {
  int n;
  int success = 0;
...
static int demangle_class(struct work_stuff *work, const char **mangled,
                          string *declp) {
  int success = 0;
  int btype;
  string class_name;
...
static int demangle_prefix(struct work_stuff *work, const char **mangled,
                           string *declp) {
  int success = 1;
  const char *scan;
  int i;

  if (strlen(*mangled) > 6 && (strncmp(*mangled, "_imp__", 6) == 0 ||
...
static int gnu_special(struct work_stuff *work, const char **mangled,
                       string *declp) {
  int n;
  int success = 1;
  const char *p;

  if ((*mangled)[0] == '_' && strchr(cplus_markers, (*mangled)[1]) != NULL &&
      (*mangled)[2] == '_') {
    /* Found a GNU style destructor, get past "_<CPLUS_MARKER>_" */
    (*mangled) += 3;
    work->destructor += 1;
  } else if ((*mangled)[0] == '_' &&
             (((*mangled)[1] == '_' && (*mangled)[2] == 'v' &&
               (*mangled)[3] == 't' && (*mangled)[4] == '_') ||
              ((*mangled)[1] == 'v' && (*mangled)[2] == 't' &&
...
static void recursively_demangle(struct work_stuff *work, const char **mangled,
...
static int demangle_qualified(struct work_stuff *work, const char **mangled,
                              string *result, int isfuncname, int append) {
  int qualifiers = 0;
  int success = 1;
  const char *p;
...
static int do_type(struct work_stuff *work, const char **mangled,
                   string *result) {
  int n;
  int done;
  int success;
  ...
  while (success && !done) {
    ...
    case 'A': {
      ...
      if (**mangled != '_')
        success =
            demangle_template_value_parm(work, mangled, &decl, tk_integral);
      if (**mangled == '_')
        ++(*mangled);
      string_append(&decl, "]");
      break;
...
static int demangle_fund_type(struct work_stuff *work, const char **mangled,
                              string *result) {
  int done = 0;
  int success = 1;
  char buf[10];
...
static int do_hpacc_template_const_value(struct work_stuff *work,
...
static int do_arg(struct work_stuff *work, const char **mangled,
                  string *result) {
...
static void remember_type(struct work_stuff *work, const char *start, int len) {
  ...
  if (work->ntypes >= work->typevec_size) {
    if (work->typevec_size == 0) {
      ...
      work->typevec_size *= 2;
      work->typevec = (char **)xrealloc((char *)work->typevec,
...
static void remember_Ktype(struct work_stuff *work, const char *start,
                           int len) {
  char *tem;
  ...
  if (work->numk >= work->ksize) {
    if (work->ksize == 0) {
      ...
      work->ksize *= 2;
      work->ktypevec = (char **)xrealloc((char *)work->ktypevec,
...
static int register_Btype(struct work_stuff *work) {
  ...
  if (work->numb >= work->bsize) {
    if (work->bsize == 0) {
      ...
      work->bsize *= 2;
      work->btypevec = (char **)xrealloc((char *)work->btypevec,
...
static void remember_Btype(struct work_stuff *work, const char *start, int len,
                           int index) {
  char *tem;
...
static int demangle_args(struct work_stuff *work, const char **mangled,
                         string *declp) {
  string arg;
  int need_comma = 0;
  int r;
...
static int demangle_nested_args(struct work_stuff *work, const char **mangled,
                                string *declp) {
...
static void demangle_function_name(struct work_stuff *work,
  ...
  *(declp->p) = '\0';
  ...
  if (declp->p - declp->b >= 3 && declp->b[0] == 'o' && declp->b[1] == 'p' &&
  ...
  } else if (declp->b[0] == '_' && declp->b[1] == '_' && declp->b[2] == 'o' &&
             declp->b[3] == 'p') {
    /* ANSI.  */
    /* type conversion operator.  */
    tem = declp->b + 4;
  ...
  } else if (declp->b[0] == '_' && declp->b[1] == '_' && declp->b[2] >= 'a' &&
             declp->b[2] <= 'z' && declp->b[3] >= 'a' && declp->b[3] <= 'z') {
    if (declp->b[4] == '\0') {
      /* Operator.  */
      for (i = 0; i < sizeof(optable) / sizeof(optable[0]); i++) {
        if (strlen(optable[i].in) == 2 &&
...
static void string_init(string *s) { s->b = s->p = s->e = NULL; }

static void string_clear(string *s) { s->p = s->b; }
